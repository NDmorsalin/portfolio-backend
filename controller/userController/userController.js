...
const updateUser = catchAsyncError(async (req, res, next) => {
  ...
  const familiarTechToSave =
    familiarTech && familiarTech.length === 0
      ? forUpdateUser.familiarTech
      : familiarTech;

  const skillsToSave =
    skills && skills.length === 0 ? forUpdateUser.skills : skills;

  // todo send string from frontEnd
  // profile image and thump image
  const avatarFile = req?.files?.find((file) => file.fieldname === "avatar");
  const thumpPicFile = req?.files?.find(
    (file) => file.fieldname === "thumpPic"
  );
